...
var (
	...
	ask_user *bool = flag.Bool("y", false, "Ask user for insert confirmation of endpoints one by one.")
...
func main() {
	...
	fmt.Println("Inserting the list of provided subscriptions...")
	...
	for _, sub := range subscriptions {
		...
		if !*ask_user {
			...
			fmt.Println(fmt.Sprintf("Do you want to insert subscription: %s ?", uaid))
			_, _ = reader.ReadString('\n')
		}

		fmt.Println("Inserting ...")

		inserted := InsertUser(uaid,chid,cass_session)
		if inserted == false {

			fmt.Println("Error: Cannot insert user: ", uaid)

			return

		}
		fmt.Println(fmt.Sprintf("uaid: %s, is inserted.", uaid))

	}

	fmt.Println("All subscriptions are successfully inserted.")
}
...
func InsertUser(uaid string,chid string, session *gocql.Session) (inserted bool) {

	var err error

	inserted = false

	fmt.Println("Inserting UAID:", uaid)
	fmt.Println("Inserting CHID:", chid)

	if err = session.Query("INSERT INTO autopush.router (uaid, connected_at , current_month , last_connect , node_id , record_version , router_type ) VALUES (?, 1538092209661,'message_2018_10',201809140002,'http://localhost.localdomain:8081',1,'webpush')",uaid).Consistency(gocql.One).Exec(); err != nil {
		fmt.Println(err)
		return false
	...
	query_str :=fmt.Sprintf("UPDATE autopush.message_2018_10 SET chids = chids + {'%s'} WHERE uaid= ? AND chidmessageid=''", chid)
	
	if err = session.Query(query_str, uaid).Consistency(gocql.One).Exec(); err != nil {
		fmt.Println(err) 
		return false	
	}
