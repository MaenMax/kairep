...
import (
	...
	"empowerthings.com/autoendpoint/routercore/webpush"
...
func WebPushHandler(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	if api_ver == "v1" {
		...
		webpush.Route_Webpush_Message(encrypted_endpoint, body, r.Header, crypto_key, debug, db_session, w, stats, is_limited)
		...
		router_worker := routercore.NewRouterWorker(encrypted_endpoint, body, r.Header, crypto_key, debug, db_session, w, 1, true, stats, false, is_limited)
...
func SimplePushHandler(w http.ResponseWriter, r *http.Request) {
	...
	router_worker := routercore.NewRouterWorker(encrypted_endpoint, body, r.Header, crypto_key, debug, db_session, w, 1, false, stats, false, is_limited)
	router_worker.RouteNotification()
...
func Route_Vapid_Webpush_Message(w http.ResponseWriter, r *http.Request) {
	...
	router_worker := routercore.NewRouterWorker(encrypted_endpoint, body, r.Header, crypto_key, debug, db_session, w, 2, true, stats, false, is_limited)
	...
	*/
	...
	for _, encrypted_endpoint := range multicast_list {
		...
		if len(encrypted_endpoint) == 183 { // vapid endpoint
			...
			router_worker := routercore.NewRouterWorker(encrypted_endpoint, message, r.Header, crypto_key, debug, db_session, w, 1, true, stats, true, is_limited)
			router_worker.RouteNotification()

		} else { //Webpush endpoint

			router_worker := routercore.NewRouterWorker(encrypted_endpoint, message, r.Header, crypto_key, debug, db_session, w, 2, false, stats, true, is_limited)
			router_worker.RouteNotification()
