...
var MALFORMED_CURRENT_MONTH_VALUE  = errors.New("Malformed current_month value")
...
// This function takes the name of message table as an argument, and then returns the int values of the month/year the message table was created.
func GetMonthAndYear(message_table_name string)(month int, year int, err error){
	str:=strings.Split(message_table_name,"_")
	if len(str)!=3{
		err= MALFORMED_CURRENT_MONTH_VALUE
		return
	}
	str_year:=  str[1]
	str_month:= str[2]
	year,  err = strconv.Atoi(str_year)
	if err!=nil{
		return
	}
