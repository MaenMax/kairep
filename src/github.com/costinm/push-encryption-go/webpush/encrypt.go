...
var (
	...
	//~ }
        // rfc test
	rfc_salt string = string("lngarbyKfMoi9Z75xYXmkg")
	randomSalt = func() ([]byte, error) {
		salt, err := base64.RawURLEncoding.DecodeString(rfc_salt)
		if err != nil {
			return nil, err
		}
		return salt, nil
...
func debug_crypto(id string, val []byte) {

	fmt.Printf("DGB-CRYPTO: Key: \"%s\" - Value: \"%s\"\n", id, base64.RawURLEncoding.EncodeToString(val))
}
